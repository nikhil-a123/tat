...
void lv_settings_create(lv_obj_t *root, lv_settings_page_t *pages, uint8_t num_pages,
        ...
        for (int j = 0; j < pages[i].num_items; j++) {
            ...
            switch (item->type) {
                case LV_SETTINGS_TYPE_SWITCH:
                    obj = create_switch(sub_page, item->item.sw.name, *item->item.sw.inital_val);
                    lv_obj_add_event_cb(obj, switch_event_cb, LV_EVENT_ALL, item->change_callback);
                    break;
                case LV_SETTINGS_TYPE_SLIDER:
                    obj = create_slider(sub_page, item->item.slider.name, item->item.slider.min_val, item->item.slider.max_val,
