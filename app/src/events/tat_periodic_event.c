...
int tat_periodic_chan_add_obs(const struct zbus_channel *chan, const struct zbus_observer *obs)
{
...
int tat_periodic_chan_rm_obs(const struct zbus_channel *chan, const struct zbus_observer *obs)
{
    struct k_work_delayable *work = NULL;
...
static int tat_timer_init(void)
{
    struct k_work_delayable *work = NULL;
...
SYS_INIT(tat_timer_init, APPLICATION, 1);
